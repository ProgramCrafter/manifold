...
function getSearchContractWhereSQL(args: {
  ...
  const filterSQL: FilterSQL = {
    ...
    closed: `close_time <= NOW() AND resolution_time IS NULL AND outcome_type != 'BOUNTIED_QUESTION'`,
    ...
    resolved: `resolution_time IS NOT NULL OR (outcome_type = 'BOUNTIED_QUESTION' AND close_time <= NOW())`,
    all: '',
  }
